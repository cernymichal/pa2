...
static bool whitespaceMatch(const T_& x, const std::string& ref) {
...
int main(void) {
    ...
    assert(whitespaceMatch(a.field("m_Status"),
                           "enum\n"
                           "{\n"
                           "  NEW,\n"
                           "  FIXED,\n"
    ...
    assert(whitespaceMatch(b.field("m_Other"),
                           "struct\n"
                           "{\n"
                           "  int m_Length;\n"
                           "  enum\n"
    ...
    assert(whitespaceMatch(b.field("m_Other").field("m_Status"),
                           "enum\n"
                           "{\n"
                           "  NEW,\n"
                           "  FIXED,\n"
    ...
    try {
        std::cout << a.field("m_Length").field("m_Foo") << std::endl;
        assert("field: missing exception!" == nullptr);
    } catch (const std::invalid_argument& e) {
    ...
    try {
        std::cout << a.field("m_Status").field("m_Foo") << std::endl;
        assert("field: missing exception!" == nullptr);
    } catch (const std::invalid_argument& e) {
        assert(whitespaceMatch(e.what(),
                               "Cannot use field() for type: enum\n"
                               "{\n"
                               "  NEW,\n"
                               "  FIXED,\n"
                               "  BROKEN,\n"
    ...
    assert(whitespaceMatch(ar5,
                           ...
                           "  READY\n"
                           "}*"));
    assert(whitespaceMatch(ar5.element(),
                           "enum\n"
                           "{\n"
                           "  NEW,\n"
                           "  FIXED,\n"
    ...
    a.addField("m_Ar1", ar1).addField("m_Ar2", ar2).addField("m_Ar3", ar3).addField("m_Ar4", ar4).addField("m_Ar5", ar5).addField("m_Ar6", ar6).addField("m_Ar7", ar7).addField("m_Ar8", ar8).addField("m_Ar9", ar9);
    assert(whitespaceMatch(a,
                           "struct\n"
                           "{\n"
                           "  int m_Length;\n"
    ...
    try {
        std::cout << ar2.field("m_Foo") << std::endl;
        assert("field: missing exception!" == nullptr);
    } catch (const std::invalid_argument& e) {
    ...
    try {
        ...
        assert(whitespaceMatch(e.what(),
                               "Cannot use element() for type: struct\n"
                               "{\n"
                               "  int m_First;\n"
                               "  enum\n"
                               "  {\n"
